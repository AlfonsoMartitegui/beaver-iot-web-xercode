...
import { zhCN, enUS, esES, jaJP, type Localization } from '@mui/material/locale';
...
export const langs: LangListType = {
    ...
    ES: {
        key: LANGUAGE.ES,
        value: i18nHelper.getComponentLanguage(LANGUAGE.ES, 'dayjs'),
        muiLocale: esES,
        labelIntlKey: 'common.language.es',
    },
    JA: {
        key: LANGUAGE.JA,
...
export const initI18n = async (platform: AppType, defaultLang?: LangType) => {
    ...
    if (!lang || !langs[lang]) {
        ...
        /**
         * Match Spanish characters es / es_es / es_mx
         */
        if (/^es(_\w+)?/i.test(language)) {
